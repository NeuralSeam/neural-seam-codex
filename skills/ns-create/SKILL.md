---
name: ns-create
description: "Neural Seam: cenario greenfield (o projeto ainda nao existe). Mostra o link do wizard da bridge local para o dev criar o projeto. Ative quando o dev quiser criar um projeto novo no Neural Seam, ou pedir /ns-create."
---
...
2. Trate o `status`:
   ...
   - `needs_connect` -> o projeto ja existe no Neural Seam; nao precisa criar. Redirecione para `/ns-connect`.
   - `needs_login` -> peca `neural-seam login` primeiro.
   - `ok` -> ja ha projeto vinculado a este diretorio; nada a criar. Sugira `/ns-list`.
3. Feche com o proximo passo: **apos criar no wizard, rode `/ns-status`** (o estado deve virar `ok`) e
   siga para `/ns-generate`.

Se `check_setup` retornar `error`, oriente `/ns-doctor`.
