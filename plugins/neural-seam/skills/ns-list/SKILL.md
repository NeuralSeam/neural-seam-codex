---
name: ns-list
description: "Neural Seam: lista as atividades/cards do projeto conectado, agrupadas por status, com o id pronto para colar. Aceita filtro opcional de status (BACKLOG, IN_PROGRESS, BLOCKED, DONE) e de kind (TECH, FEATURE, IMPROVE, BUG, VERIFY). Ative quando o dev perguntar o que ha para fazer, quiser ver os cards/backlog, ou pedir $neural-seam:ns-list."
---
...
2. Apresente os cards **agrupados por status** (a fazer / em progresso / bloqueado / concluido). Para
   ...
   `blocked_by`.
3. Feche indicando o proximo passo: **`$neural-seam:ns-exec <id>`** para implementar um card, ou `$neural-seam:ns-open` para
