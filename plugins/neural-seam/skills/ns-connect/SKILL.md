---
name: ns-connect
description: "Neural Seam: vincula um projeto JA existente a este diretorio (materializa o manifesto/binding, sem clonar codigo). Ative quando o dev disser que ja criou o projeto no wizard e quer liga-lo a esta pasta, quando check_setup retornar needs_connect, ou quando pedir /ns-connect."
---
...
1. **Caminho guiado (recomendado, sem digitar id):** chame a tool `check_setup` do servidor MCP
   ...
   - Se `status = needs_setup`, nao ha projeto para conectar -> redirecione para `/ns-create`.
   - Se `status = ok`, ja esta vinculado -> sugira `/ns-list`.
...
3. Feche com o proximo passo: **apos vincular, rode `/ns-status`**; com `ok`, siga para `/ns-generate`
   (bootstrap) ou `/ns-clone <id>` se ainda faltar o codigo.

Se algo falhar por auth/rede, mostre a mensagem e sugira `neural-seam login` ou `/ns-doctor`.
